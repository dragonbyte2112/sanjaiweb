...
app.use(
  cors({
    origin: (origin, cb) => {
      ...
      // since the frontend is served from this same Express app (local dev)
      // or the same Vercel domain (production).
...
// Serve the plain HTML/CSS/JS frontend from the same origin. On Vercel this
// code path is never actually reached (Vercel's static CDN serves the
// public-site files directly for non-/api paths), but it's kept so local
// `npm run dev` behaves identically to before.
app.use(express.static(staticSiteDir));

// Allow clean URLs without ".html" (e.g. /admin -> admin.html) locally.
...
// Run once per cold start (local process, or a fresh Vercel function
// instance) — checks/seeds the admin account and starter content.
let dbReady = initDb().catch((err) => {
  console.error("initDb failed:", err.message);
});
export const whenDbReady = () => dbReady;
